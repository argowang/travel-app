                ...
                ForEach(self.eventCards, id: \.uuid) { card in
                    ZStack(alignment: Alignment(horizontal: .leading, vertical: .top)) {
                        if self.mode?.wrappedValue == .active {
                            ...
                            NavigationLink(destination: AddEventView(selectedDate: card.start ?? Date(), selectedTime: card.start ?? Date(), type: EventType(rawValue: card.type ?? EventType.general.rawValue), rating: Int(card.rating), card, card: card as! EventCard, place: Place(card.title ?? "", CLLocationCoordinate2D(latitude: card.latitude, longitude: card.longitude))), tag: card.uuid!, selection: self.$selected) {
                                EventCardView(title: card.title ?? "title place holder", type: EventType(rawValue: card.type ?? EventType.general.rawValue), dateString: self.dateFormatter.string(from: card.start ?? Date()))
                                    .onTapGesture {
